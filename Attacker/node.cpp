...
Node::Node() {
	...
	console = make_shared<Console>(shared_ptr<Node>(this));
...
vector<int> Node::findOnionPath(int target_id) {
	queue<vector<int>> queue;
	...
	while (!queue.empty()) {
		...
		for (shared_ptr<DHTConnection>& connection : current_node->connections) {
			...
			for (int i = 0; i < current_vector.size(); i++) {
				if (current_vector[i] == next_node->id) {
					been_check = true;
					break;
				}
			}
			...
			if (!been_check) {
				...
				if (next_node->id == target_id) paths.push_back(new_vector);
				else queue.push(new_vector);
			}
		}
	}

	if (paths.size() == 0) {
		cout << "error in findOnionPath" << endl; // debug
		return vector<int>();
	}
	
	int index = generateRandom(0, paths.size() - 1);
	
	// debug
	cout << "<";
	for (int i = 0; i < paths[index].size(); i++) {
		cout << paths[index][i];
		if (i < paths[index].size() - 1) cout << ", ";
	}
	cout << ">" << endl;
...
void Node::sendMPNP(int target_id, string payload) {
	vector<int> path = findOnionPath(target_id);
